...
void cGame::initialise(SDL_Window* theSDLWND, SDL_Renderer* theRenderer)
{
	...
	// Store the textures
	textureName = { "asteroid1", "asteroid2", "asteroid3", "asteroid4", "photon", "bullet", "theBody", "theRocket","theBackground", "explosion"};
	texturesToUse = { "Images\\Sprites\\Armoured_1.png", "Images\\Sprites\\Armoured_2.png", "Images\\Sprites\\Mech_1.png", "Images\\Sprites\\Mech_2.png", "Images\\Sprites\\Mech_3.png", "Images\\Sprites\\shell.png", "Images\\Sprites\\PlayerBody.png", "Images\\Sprites\\PlayerTurret.png", "Images\\Bkg\\Mars_2.png", "Images\\Sprites\\explosion_4.png" };
	for (int tCount = 0; tCount < (int)textureName.size(); tCount++)
	{	
		theTextureMgr->addTexture(textureName[tCount], texturesToUse[tCount]);
	}
	// Create textures for Game Dialogue (text)
	fontList = { "digital", "bebas" };
	fontsToUse = { "Fonts/digital-7.ttf", "Fonts/BebasNeue_Regular.ttf" };
	for (int fonts = 0; fonts < (int)fontList.size(); fonts++)
	{
		theFontMgr->addFont(fontList[fonts], fontsToUse[fonts], 36);
	}
	gameTextList = { "Mohammed Zeeshan", "Score : ","Controls: left and right arrow keys to rotate and space to fire"};
	strScore = gameTextList[1];
	strScore += to_string(theScore).c_str();
	
	theTextureMgr->addTexture("Title", theFontMgr->getFont("bebas")->createTextTexture(theRenderer, gameTextList[0], textType::solid, { 0, 255, 0, 255 }, { 0, 0, 0, 0 }));
	theTextureMgr->addTexture("theScore", theFontMgr->getFont("bebas")->createTextTexture(theRenderer, strScore.c_str(), textType::solid, { 0, 255, 0, 255 }, { 0, 0, 0, 0 }));
	theTextureMgr->addTexture("theInstructions", theFontMgr->getFont("bebas")->createTextTexture(theRenderer, gameTextList[2], textType::solid, { 0, 255, 0, 255 }, { 0, 0, 0, 0 }));

	// Load game sounds
	soundList = { "theme", "shot", "explosion" };
	soundTypes = { soundType::music, soundType::sfx, soundType::sfx };
	soundsToUse = { "Audio/Theme.wav", "Audio/shot.wav", "Audio/explosion4.wav" };
	...
	theBody.setSpritePos({ 450, 600 });
	theBody.setTexture(theTextureMgr->getTexture("theBody"));
	theBody.setSpriteDimensions(theTextureMgr->getTexture("theBody")->getTWidth(), theTextureMgr->getTexture("theBody")->getTHeight());
	theBody.setRocketVelocity(100);
	theBody.setSpriteTranslation({ 50,0 });

	theRocket.setSpritePos({ 480, 560 });
	...
	//	theAsteroids[astro]->setSpritePos({ (50 * (astro + 1)+(50*astro)), -75 });
	//	theAsteroids[astro]->setSpriteTranslation({ 0, -100 });
	...
	//	theAsteroids[astro]->setSpriteRotAngle((float)(theAsteroids[astro]->getSpriteRotAngle() + 90));
...
void cGame::render(SDL_Window* theSDLWND, SDL_Renderer* theRenderer)
{
	...
	if (updateScore)
	{
		...
		theTextureMgr->addTexture("theScore", theFontMgr->getFont("bebas")->createTextTexture(theRenderer, strScore.c_str(), textType::solid, { 255, 255, 255, 255 }, { 255, 255, 255, 255 }));
		updateScore = false;
	}
	...
	// render the rocket
	theBody.render(theRenderer, &theBody.getSpriteDimensions(), &theBody.getSpritePos(), theBody.getSpriteRotAngle(), &theBody.getSpriteCentre(), theBody.getSpriteScale());
...
void cGame::update(double deltaTime)
{
	...
	*/
	for (vector<cBullet*>::iterator bulletIterartor = theBullets.begin(); bulletIterartor != theBullets.end(); ++bulletIterartor)
	{
		...
		for (vector<cAsteroid*>::iterator asteroidIterator = theAsteroids.begin(); asteroidIterator != theAsteroids.end(); ++asteroidIterator)
		{
			if ((*asteroidIterator)->collidedWith(&(*asteroidIterator)->getBoundingRect(), &(*bulletIterartor)->getBoundingRect()))
			{
				...
				theExplosions[index]->setNoFrames(8);
				...
				theExplosions[index]->setSpritePos({ (*asteroidIterator)->getSpritePos().x + (int)((*asteroidIterator)->getSpritePos().w/2) - 210, (*asteroidIterator)->getSpritePos().y + (int)((*asteroidIterator)->getSpritePos().h / 2) - 200 });
	...
	if (frames % 360 == 0)
	{
		...
		{
			theAsteroids.push_back(new cAsteroid);
			theAsteroids[astro]->setSpritePos({ (100 * count + (100 * (count - 1))), -75 });
			theAsteroids[astro]->setSpriteTranslation({ 0, -100 });
			int randAsteroid = AsteroidTextDis(gen);
			theAsteroids[astro]->setTexture(theTextureMgr->getTexture(textureName[randAsteroid]));
			theAsteroids[astro]->setSpriteDimensions(theTextureMgr->getTexture(textureName[randAsteroid])->getTWidth(), theTextureMgr->getTexture(textureName[randAsteroid])->getTHeight());
		...
		switch (event.type)
		{
			...
			case SDL_KEYDOWN:
				...
				case SDLK_SPACE:
				{
					...
					theBullets[numBullets]->setSpritePos({ theRocket.getBoundingRect().x + theRocket.getSpriteCentre().x - 50, theRocket.getBoundingRect().y + theRocket.getSpriteCentre().y - 100 });
